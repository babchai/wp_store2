...
// THE VISITOR HAS CLICKED THE CHECKOUT BUTTON
	...
	*/
	...
	if(trim($_POST['shipping_add_id']) == "")
	{
		header('Location: ' . $_POST['wpsc_checkout_page'] .'?err=Shipping Address Is Empty!');
		exit;
	...
	// SEND CART CONTENTS TO PAYPAL USING THEIR UPLOAD METHOD, FOR DETAILS SEE http://tinyurl.com/djoyoa
	else if ($valid_prices === true)
		{
                ...
                if($paymentGateway == 'checkmoneyorder') {
                    ...
                    `wpuser`, `email`, `affiliate`, `date`, `address_id`) VALUES
                    (NULL, 'Pending', '{$cartContents}', 'Check/Money Order', '{$totalPrice}', '{$totalShipping}', '{$purchaser_user_id}', '{$purchaser_email}', '{$affiliateid}', '{$timestamp}','{$_POST['shipping_add_id']}');
                    ";
                ...
                if($paymentGateway == 'paypal') {
                    ...
                    (`primkey`, `orderstatus`, `cartcontents`, `paymentprocessor`, `price`, `shipping`,
                    `wpuser`, `email`, `affiliate`, `date` , `address_id`) VALUES
                    (NULL, 'Pending', '{$cartContents}', 'PayPal', '{$totalPrice}', '{$totalShipping}', '{$purchaser_user_id}', '{$purchaser_email}', '{$affiliateid}', '{$timestamp}', {$_POST['shipping_add_id']});
                    ";
